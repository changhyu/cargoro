...
class DrivingRecordFilters(BaseModel):
    ...
    driver_id: Optional[str] = Field(None, description="운전자 ID")
    vehicle_id: Optional[str] = Field(None, description="차량 ID")
    start_time_from: Optional[datetime] = Field(
        None, description="운행 시작 시간 범위 시작"
    )
    start_time_to: Optional[datetime] = Field(
        None, description="운행 시작 시간 범위 종료"
    )
    min_distance: Optional[float] = Field(None, description="최소 주행 거리", ge=0)
    max_distance: Optional[float] = Field(None, description="최대 주행 거리", ge=0)
    min_duration: Optional[int] = Field(None, description="최소 운행 시간(분)", ge=0)
    max_duration: Optional[int] = Field(None, description="최대 운행 시간(분)", ge=0)


class DrivingRecordListResponse(BaseModel):
    """운행 기록 목록 응답 모델"""

    items: List[DrivingRecordResponse] = Field(..., description="운행 기록 목록")
...
class DriverPerformanceBase(BaseModel):
    ...
    efficiency_score: Optional[int] = Field(
        None, description="효율성 점수(0-100)", ge=0, le=100
    )
