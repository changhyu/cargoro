...
class MaintenancePartUpdate(BaseModel):
    ...
    name: Optional[str] = Field(None, description="부품 이름")
    part_number: Optional[str] = Field(None, description="부품 번호")
    quantity: Optional[int] = Field(None, description="수량", gt=0)
    unit_price: Optional[float] = Field(None, description="단가", ge=0)
    total_price: Optional[float] = Field(None, description="총 금액", ge=0)
...
class MaintenanceCreate(MaintenanceBase):
    ...
    parts: Optional[List[MaintenancePartCreate]] = Field(
        None, description="교체/수리된 부품 목록"
    )
...
class MaintenanceUpdate(BaseModel):
    ...
    parts: Optional[List[MaintenancePartUpdate]] = None
...
class MaintenanceScheduleSettings(BaseModel):
    ...
    regular_service_interval_km: int = Field(
        ..., description="정기 점검 간격(km)", ge=0
    )
    regular_service_interval_days: int = Field(
        ..., description="정기 점검 간격(일)", ge=0
    )
    ...
    tire_rotation_interval_km: int = Field(
        ..., description="타이어 위치 교환 간격(km)", ge=0
    )
    brake_check_interval_km: int = Field(
        ..., description="브레이크 점검 간격(km)", ge=0
    )
    notification_days_before: int = Field(7, description="사전 알림 일수", ge=0)
    vehicle_id: Optional[str] = Field(
        None, description="특정 차량 ID(없으면 기본 설정)"
    )
