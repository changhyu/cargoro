...
# 데이터베이스 임포트 수정
from database.db_operations import get_prisma, Prisma
...
async def create_vehicle(vehicle: VehicleCreate, prisma: Prisma = Depends(get_prisma)):
    """
    새로운 차량을 등록합니다.
    """
    try:
        ...
        return create_response(
            data=created_vehicle, message="차량이 성공적으로 등록되었습니다."
        )
...
async def get_vehicle_by_id(
    ...
    try:
        ...
        return create_response(
            data=vehicle, message="차량 정보가 성공적으로 조회되었습니다."
        )
...
async def update_vehicle(
    ...
    try:
        ...
        if (
            ...
            and vehicle_data.license_plate != existing_vehicle.licensePlate
        ):
        ...
        if vehicle_data.vin and vehicle_data.vin != existing_vehicle.vin:
            duplicate_vin = await prisma.vehicle.find_first(
                where={"vin": vehicle_data.vin, "id": {"not": vehicle_id}}
            )
            if duplicate_vin:
                raise conflict_exception(RESOURCE_TYPE, "vin")
        ...
        return create_response(
            data=updated_vehicle, message="차량 정보가 성공적으로 업데이트되었습니다."
        )
...
async def delete_vehicle(
    ...
    try:
        ...
        return create_response(
            data={"id": vehicle_id}, message="차량이 성공적으로 삭제되었습니다."
        )
    ...
    try:
        ...
        if status not in valid_statuses:
            raise validation_exception(
                {
                    "status": f"유효하지 않은 상태입니다. 가능한 값: {', '.join(valid_statuses)}"
                }
            )

        # 상태 업데이트
        updated_vehicle = await prisma.vehicle.update(
            where={"id": vehicle_id}, data={"status": status}
        )

        return create_response(
            data=updated_vehicle, message=f"차량 상태가 '{status}'로 변경되었습니다."
        )
...
async def get_assignments_by_vehicle(
    ...
    try:
        ...
        # 배정 목록 조회
        assignments = await prisma.vehicle_assignment.find_many(
            where={"vehicle_id": vehicle_id}, include={"driver": True}
        )

        return create_response(
            data=assignments, message="차량 배정 목록이 성공적으로 조회되었습니다."
        )
