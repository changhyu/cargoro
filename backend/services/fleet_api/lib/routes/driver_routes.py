...
async def create_driver(
    ...
    try:
        ...
        if (
            "emergency_contact" in driver_data_dict
            and driver_data_dict["emergency_contact"]
        ):
            driver_data_dict["emergencyContact"] = driver_data_dict[
                "emergency_contact"
            ].model_dump()
        ...
        if "profile_image_url" in driver_data_dict:
            driver_data_dict["profileImageUrl"] = driver_data_dict.pop(
                "profile_image_url"
            )
        ...
        return create_response(
            data=new_driver, message="운전자가 성공적으로 생성되었습니다."
        )
...
async def get_drivers(
    ...
    try:
        ...
        logger.error(f"예상치 못한 오류: {str(e)}")
        raise server_error_exception(
            f"운전자 목록 조회 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_driver_by_id(
    ...
    try:
        ...
        return create_response(
            data=driver, message="운전자 정보가 성공적으로 조회되었습니다."
        )
...
@router.get(
    "/drivers/license-expiry/alerts", response_model=ApiResponse[List[DriverResponse]]
)
async def get_license_expiry_alerts(
    ...
    try:
        ...
        logger.error(f"Prisma 오류: {str(e)}")
        raise server_error_exception(
            "면허 만료 알림 조회 중 데이터베이스 오류가 발생했습니다."
        )
    except ApiException as e:
        raise
    except Exception as e:
        logger.error(f"예상치 못한 오류: {str(e)}")
        raise server_error_exception(
            f"면허 만료 알림 조회 중 오류가 발생했습니다: {str(e)}"
        )
